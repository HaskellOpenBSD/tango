...
def category(request, category_name_url):
	...
	# Add the name of the category passed by the user.
...
def add_category(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			# Thu user will be shown the homepage.
...
def add_page(request, category_name_url):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			# Wrap the code in a try block - check if the category
			# actually exists!
			try:
				...
				# saying the category does not exist.
...
def register(request):
	...
	# A boolean value for telling the templat whether the registration
	# was successful. Set to False initially.  Code changes to True
	# when registered.
	registered = False

	# If it's a HTTP POST, we're interested in processing form data.
	if request.method == 'POST':
		# Attempt to grab info from the row form information.
		...
		if user_form.is_valid() and profile_form.is_valid():
			...
			# commit = False.  This delays saving the model until we're
...
def user_login(request):
	...
	if request.method == 'POST':
		...
		# password combo is valid - a User object is returned if it is.
		...
		if user is not None:
			...
			if user.is_active:
				...
				# user in.  We'll send them back to the homepage.
				login(request, user)
			...
			# Bad login details were provided, we can't log them in.
			print('Invalid login details: {0}, {1}'.format(username, 
				password))
			return HttpResponse('Invalid login details: username {0},' 
				'password {1}'.format(username, password))

	
	# The request is not a HTTP POST, so display the login form.
	# This is most likely a HTTP GET.
	else:
